...
class FollowerList extends StatelessWidget {
  ...
  const FollowerList({Key? key, required this.image, required this.name, required this.phoneNum}) : super(key: key);
          ...
          children:[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                  color: const Color(0xFF515151).withOpacity(.8),
                  borderRadius: BorderRadius.circular(MediaQuery.of(context).size.height*.01)
              ),
              child: Center(child: Text("Remove",style: GoogleFonts.plusJakartaSans(color: Colors.black,fontSize:16,fontWeight: FontWeight.w600),)),
            )
          ]
      ),
    );
